...
func Test_GetFileContentsWithAllowRawFallback(t *testing.T) {
	t.Parallel()

	// Mock content for the file
	mockFileContent := "Hello, GitHub Copilot!"
	mockFileSHA := "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0"
	...
	tests := []struct {
		...
		{
			name:          "allow_raw_fallback is true (default behavior)",
			allowFallback: boolPtr(true),
			mockResponses: []mock.MockBackendOption{
				// First, attempt to get content with application/vnd.github.v3+json
				mock.WithRequestMatchHandler(
					mock.GetReposContentsByOwnerByRepoByPath,
					http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						assert.Contains(t, r.Header.Get("Accept"), "application/vnd.github.v3+json")
						w.WriteHeader(http.StatusNotFound) // Simulate file not found in tree endpoint
					}),
				),
				// Then, fallback to raw content
				mock.WithRequestMatchHandler(
					raw.GetRawReposContentsByOwnerByRepoByBranchByPath,
					http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						assert.Contains(t, r.Header.Get("Accept"), "application/vnd.github.v3.raw")
			...
			expectError: false,
			expectedSHA: "", // Raw content does not return SHA
			expectedContent: mockFileContent,
		},
		{
			name:          "allow_raw_fallback is false (force SHA retrieval)",
			allowFallback: boolPtr(false),
			mockResponses: []mock.MockBackendOption{
				// Only attempt to get content with application/vnd.github.v3+json
				mock.WithRequestMatchHandler(
					mock.GetReposContentsByOwnerByRepoByPath,
					http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						assert.Contains(t, r.Header.Get("Accept"), "application/vnd.github.v3+json")
						content := github.RepositoryContent{
							SHA: github.Ptr(mockFileSHA),
							Content: github.Ptr(base64.StdEncoding.EncodeToString([]byte(mockFileContent))),
						}
						w.WriteHeader(http.StatusOK)
			...
			expectError: false,
			expectedSHA: mockFileSHA,
			expectedContent: mockFileContent,
		},
		{
			name:          "allow_raw_fallback is nil (default behavior, fallback allowed)",
			allowFallback: nil,
			mockResponses: []mock.MockBackendOption{
				// First, attempt to get content with application/vnd.github.v3+json
				mock.WithRequestMatchHandler(
		...
		{
			name:          "allow_raw_fallback is false and content fetch fails",
			allowFallback: boolPtr(false),
			mockResponses: []mock.MockBackendOption{
				// Only attempt to get content with application/vnd.github.v3+json
				mock.WithRequestMatchHandler(
					...
					http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						assert.Contains(t, r.Header.Get("Accept"), "application/vnd.github.v3+json")
						w.WriteHeader(http.StatusNotFound) // Simulate file not found
						_, _ = w.Write([]byte(`{"message": "Not Found"}`))
					}),
				),
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			client := github.NewClient(mock.NewMockedHTTPClient(tc.mockResponses...))
			mockRawClient := raw.NewClient(client, &url.URL{Scheme: "https", Host: "raw.example.com", Path: "/"})
			_, handler := GetFileContents(stubGetClientFn(client), stubGetRawClientFn(mockRawClient), translations.NullTranslationHelper)
			...
			if tc.allowFallback != nil {
				requestArgs["allow_raw_fallback"] = *tc.allowFallback
			}
			...
			textContent := getTextResult(t, result)
			assert.Equal(t, tc.expectedContent, textContent.Text)
			assert.Equal(t, tc.expectedSHA, textContent.URI) // For raw content, URI contains SHA if returned, otherwise empty. This needs careful assertion.
			if tc.expectedSHA != "" {
