...
func Test_GetFileContents(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "successful text content fetch via raw fallback (default behavior)",
			...
			requestArgs: map[string]interface{}{
				...
				"path":  "README.md",
				"ref":   "refs/heads/main",
				// allow_raw_fallback is omitted to test default behavior
			},
			expectError: false,
		...
		{
			name: "successful text content fetch via raw fallback (explicitly enabled)",
			mockedClient: mock.NewMockedHTTPClient(
				mock.WithRequestMatchHandler(
					raw.GetRawReposContentsByOwnerByRepoByRef,
					http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		},
		{
			...
			requestArgs: map[string]interface{}{
				...
				"repo":               "repo",
				"path":               "src/",
				"allow_raw_fallback": false, // fallback doesn't apply to dirs, but good to test
			},
			expectError:    false,
			expectedResult: mockDirContent,
		},
		{
			name: "successful directory content fetch (fallback enabled)",
			mockedClient: mock.NewMockedHTTPClient(
				mock.WithRequestMatchHandler(
					mock.GetReposContentsByOwnerByRepoByPath,
					mock.WithPath("repos/owner/repo/contents/src/"),
			...
			requestArgs: map[string]interface{}{
				...
				"repo":  "repo",
				"path":  "src/",
				// allow_raw_fallback is omitted, should still go to API for directories
			},
			expectError:    false,
			expectedResult: mockDirContent,
		},
		{
			...
			requestArgs: map[string]interface{}{
				...
				"repo":  "repo",
				"path":  "README.md",
				// allow_raw_fallback is omitted to test default fallback
			},
			expectError:    false,
			expectedResult: mockFileContent,
		},
