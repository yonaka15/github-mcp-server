...
func Test_GetFileContents(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "successful text content fetch via raw fallback (explicitly true)",
			mockedClient: mock.NewMockedHTTPClient(
				mock.WithRequestMatchHandler(
					raw.GetRawReposContentsByOwnerByRepoByPath,
					http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		},
		{
			...
			expectError:    true,
			expectedErrMsg: "failed to get file or directory contents",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			case *github.RepositoryContent, []*github.RepositoryContent:
				...
				// Use assert.Equal to compare the JSON-unmarshaled struct with the expected struct.
				// This handles comparisons of pointers and values correctly.
